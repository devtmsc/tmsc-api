...
def auth_google(data: dict, response: Response, db: Session = Depends(get_auth_master_db)):
    try:
        ...
        token = create_access_token({'type': 'GoogleToken', 'uuid': db_user.id, "sub": db_user.email, 'phone': db_user.phone, 'fullname': db_user.fullname, 'department': db_user.department, 'position': db_user.position})
        refresh_token = create_refresh_token({'type': 'GoogleRefresh', 'uuid': db_user.id, "sub": db_user.email, 'phone': db_user.phone, 'fullname': db_user.fullname, 'department': db_user.department, 'position': db_user.position})

...
class RewardTransactionSerializer:
    @classmethod
    def serialize_list(cls, objects: List[Any] = [], context: dict = None, fields: List[str] = None):
        result = []
        ...
        for item in objects:
            ...
            transaction_type = get_field_value(item, 'transaction_type')
            update_field_value(item, 'transaction_type_name', get_value_from_dict(dictionary=REWARD_TRANSACTION_TYPE, key_path=transaction_type, default={}).get('name', ''))

...
def create(info: schemas.CustomerCreateSchema, db: Session = Depends(get_customer_master_db), api_key: str = Depends(verify_api_key),):
    try:
        ...
        db_data = db.query(CustomersModel).filter(CustomersModel.phone == phone, CustomersModel.channel == info.channel).first()
        if db_data:
            return {'code': 'existed', 'message': 'Số điện thoại đã tồn tại', 'data': db_data.id}

        new_customer = CustomersModel(fullname=info.fullname, phone=phone, channel=info.channel, status=True)
        db.add(new_customer)
        db.commit()
...
def create(info: schemas.CustomerLoginSchema, db: Session = Depends(get_customer_master_db), api_key: str = Depends(verify_api_key), commune_cache: CategoryCommuneCache = Depends(CategoryCommuneCache)):
    try:
        if (info.provider == 0) and not info.customer_id:
            raise HTTPException(status_code=422, detail={'code': MSG['422']['status_code'], 'message': 'Dữ liệu không hợp lệ, bạn chưa truyền mã khách hàng'})
        ...
        if info.provider == 0:
            # khách đăng ký
            customer = db.query(CustomersModel).filter(CustomersModel.id == info.customer_id, CustomersModel.channel == info.channel).first()
            is_existed = True
        else:
            # khách đăng nhập qua FB, Gmail
            social_customer = db.query(SocialCustomersModel).filter(SocialCustomersModel.provider == info.provider, SocialCustomersModel.provider_id == info.provider_id, SocialCustomersModel.channel == info.channel).first()
            if social_customer:
                # Đã có tài khoản
                customer = db.query(CustomersModel).filter(CustomersModel.id == social_customer.customer_id, CustomersModel.channel == info.channel).first()
                is_existed = True
                ...
                if phone:
                    ...
                    if customer:
                        ...
                        # Chưa có tài khoản
                        customer = CustomersModel(fullname=f"{info.first_name} {info.last_name}", phone=phone, channel=1, status=True, social=[social_customer])
                        db.add(customer)
                else:
        ...
        if not customer:
            raise HTTPException(status_code=404, detail={'code': MSG['404']['status_code'], 'message': 'Tài khoản khách hàng không tồn tại'})
